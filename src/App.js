                ...
                return (
                  <div key={p.id} className="list-item-wrapper">
                    ...
                    {isCustom && role === 'manager' && (
                      <button onClick={() => { if(window.confirm(`Excluir permanentemente a obra "${p.name}"?`)) deleteDoc(doc(db, 'custom_projects', p.id)); }} className="btn-delete-item">
                        <Trash2 size={20}/>
                      </button>
                    )}
                  </div>
              ...
              {[...(STAGES[selectedProject?.id] || []), ...customStages.filter(s => s.projectId === selectedProject?.id)].map(s => {
                ...
                return (
                  <div key={s.id} className="list-item-wrapper">
                    ...
                    {isCustom && role === 'manager' && (
                      <button onClick={() => { if(window.confirm(`Excluir permanentemente a etapa "${s.name}"?`)) deleteDoc(doc(db, 'custom_stages', s.id)); }} className="btn-delete-item">
                        <Trash2 size={20}/>
                      </button>
                    )}
                  </div>
